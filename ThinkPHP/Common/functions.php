...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    if(is_array($data)){
        ...
        $data=preg_replace('/(and |exec |insert |select |delete |update | count| chr| mid| master| truncate| char|declare |or | exists|drop )/i','',$data);
        $data=trim($data);
    }
    return $data;
}
...
function think_filter(&$value){
    // 过滤查询特殊字符
    $value=preg_replace('/(and |exec |insert |select |delete |update | count| chr| mid| master| truncate| char|declare |or | exists|drop )/i','',$value);
    $value=trim($value);
}